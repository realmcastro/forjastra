...
> correção aparece na própria regra. **Desde 2026-09-23 são quatro:** `fila-local-valores-de-partida.md`
> guarda o valor de partida de cada grandeza que estava em lacuna de produto (`RN-OFF-034` a `RN-OFF-038`,
> `RN-OFF-040`, `RN-NUC-080`); na mesma data `fila-local-conteudo-e-repouso.md` ganhou `RN-OFF-039` e
> `fila-local-autoridade-e-identidade.md` ganhou `RN-NUC-081`, e este arquivo ganhou `RN-NUC-082` (§5).
>
...
>
> **Nenhum número aqui.** Tamanho de fila, lote, espera, tentativa, duração: cada um é `LACUNA-OFF-nnn` com dono. Número não medido em spec de produto vira orçamento por acidente. Os valores de partida decididos em 2026-09-23, por delegação do humano, moram em `fila-local-valores-de-partida.md`, rotulados como não medidos.
>
...
**Acrescentado em 2026-09-23:** o identificador de item que **não** está no catálogo que o terminal retém não é artefato ausente nem valor a decidir. É recusa do lançamento com motivo próprio, e sem contato o terminal diz que o item não está no catálogo **que ele tem**, nunca que o item não existe — `RN-NUC-063` (`fatos-de-operacao-dominios-fechados.md` §1), que é o nascimento de `LACUNA-PER-6`. Nenhuma linha provisória nem preço informado nasce disso, porque seria compor valor que o backend não publicou.
...
| Lançar item por identificador que não corresponde a item no catálogo aplicado | núcleo | recusa | recusa | recusa | 1 (aplica; falha fechado naquela linha) | `RN-NUC-063`, `RN-NUC-013` (a) |
| Buscar item por texto no catálogo aplicado, para lançar | núcleo | integral | integral | integral | 1 (aplica) | `RN-NUC-081`, `RN-OFF-020` |
...
| Abrir e fechar turno | núcleo | degradado | degradado | degradado | 1 no fato + 4 na autoridade | `RN-NUC-062`, `RN-OFF-024` |
...
| Resolver qual versão de regra vale | `FIS` | integral | integral | integral | 1 | `RN-FIS-008`, `RN-NUC-057` |
...
O que **não** fechou: `RN-NUC-016` (texto guardado com origem declarada) não tem linha própria porque não é operação — é atributo das linhas de pedido, desconto e movimento de caixa, e sua faixa está declarada na regra (integral em D1, D2 e D3; classe 1). **Corrigido em 2026-09-23:** este parágrafo dizia que **abertura e fechamento de turno** continuavam fora da tabela e recusados pelo default de `RN-OFF-008`, sem regra dona para transcrever. A regra dona passou a existir (`RN-NUC-062`, que fechou `LACUNA-NUC-007`), e a linha entrou por transcrição dela, sem alterá-la: degradado nos três domínios, porque o fato é aditivo e a autoridade é a retida (linha 42 da matriz). Nada depende de turno, então a recusa de abrir turno por autoridade vencida não para a venda. Era a última contradição aberta com `PN-01` nesta tabela.
...
**Infeliz** ninguém olha → o produto **escala** por conta própria ao `establishment_responsible` antes do vencimento do prazo mais próximo, e o não-olhado nunca vira silêncio. `LACUNA-OFF-007` está **respondida na parte de papel** (`manager`, escopo estabelecimento, designado por `queue_owner`) e, desde 2026-09-23, **na parte de número**: com quanta antecedência escalar era `LACUNA-NUC-014`, e `RN-OFF-040` decide que a escalada acontece quando resta metade do tempo até o prazo e ninguém abriu o item.
...
### RN-NUC-082 — A referência humana da venda é única no estabelecimento por toda a vida dele: não reinicia, não é densa e não carrega significado

**Enunciado** a referência humana de `RN-NUC-003` é única no escopo do estabelecimento durante **toda a vida dele**, inclusive depois de encerrar e reabrir (`RN-NUC-059`). Ela **não reinicia** por dia, turno, sessão ou ano. É **esparsa**: faixa encerrada deixa números que nunca serão usados (`RN-OFF-006`, infeliz b e c), e lacuna na sequência não é defeito nem sinal de venda apagada. E **não carrega significado**: não diz o terminal, o dia, nem a ordem entre vendas de terminais diferentes. Comprimento e alfabeto são de `arquiteto-dados`, com uma condição: ser lida em voz alta e digitada no caixa sem ambiguidade. Fecha `LACUNA-NUC-003`.
**Motivo** a necessidade é a pessoa no balcão localizar a venda (devolução, reapresentação, conferência). Recusada: sequência densa que reinicia por dia ("venda 1, 2, 3 de hoje"). Com N terminais sem contato ela exige alocação central no instante da venda, que `RN-OFF-006` proíbe, ou faixas por dia, que param a referência do terminal que atravessa a meia-noite em `D2`; e o reinício torna a referência ambígua entre dias, obriga toda busca a pedir a data e faz a colisão entre estabelecimentos acontecer todo dia (`RN-NUC-038`, motivo). A nossa funciona com qualquer número de terminais offline, e a referência sozinha basta para achar a venda no estabelecimento. A necessidade que a sequência diária atendia de fato, um número curto para chamar quem espera, é **senha**: recurso escasso de módulo, pelo mesmo mecanismo de faixa (`RN-OFF-006`, natureza 1), e a referência da venda não é o lugar dela. O eixo é vendas ao longo da vida do estabelecimento: a referência cresce em comprimento com o logaritmo das vendas, e nunca em colisão.
**Aceite** três terminais em `D1` das 23h à 1h: toda referência é distinta, nenhuma reinicia à meia-noite, e a ordem das referências entre terminais não precisa ser a ordem das vendas. Devolução seis meses depois: o `cashier` digita só a referência e acha a venda (`RN-NUC-038`), sem informar data. Estabelecimento encerrado e reaberto: nenhuma referência anterior é emitida de novo.
**Infeliz** a referência fica longa demais para ler bem → é formato, de `arquiteto-dados`, e vale para vendas **novas**; referência emitida nunca muda (`PN-07`). Faixa esgotada: `RN-NUC-003`, infeliz (a).
**Offline** integral; classe 1 no fato e 3 na referência, como `RN-NUC-003`. **Registra** a referência em cada venda (`RN-NUC-003`) e o encerramento de faixa com os números não usados (`RN-OFF-006`, infeliz c). **Não registra** contador de referência por dia: é derivável dos instantes e convidaria a reiniciar.

### RN-OFF-014 — Existe teto de operação offline, declarado, e alguém o decide

**Enunciado** o terminal **não** acumula pendência indefinidamente: existe teto declarado, medido em **vendas concluídas pendentes de sincronização** (não em bytes, não em minutos), a partir do qual o produto **recusa continuar** a operar offline. É configuração do cliente, com padrão que **o humano** decide (`LACUNA-OFF-004`); decidido em 2026-09-23, por delegação: 1.000 vendas pendentes por terminal no padrão, entre 100 e 5.000, aviso em 80 % (`RN-OFF-037`).
...
**Infeliz** a divergência cruza fronteira de vigência, de turno ou de dia → o fato **não** é silenciosamente atribuído: vira pendência nomeada para o dono da fila, com as duas interpretações à vista. A fronteira é a do fuso do **estabelecimento**, publicado e com vigência (`RN-NUC-057`). **Corrigido em 2026-09-23:** esta cláusula dizia que havia conflito aberto entre o fuso do estabelecimento (`RN-FIS-008`) e o do cliente (`glossario.md` §1.1, `.claude/rules/dados.md` §3), e não escolhia lado; `RN-NUC-057` escolheu o estabelecimento e fechou `LACUNA-OFF-006`. `.claude/rules/dados.md` §3 ainda não foi alterado.
...
uma célula da §4 mudou de valor. Fechadas até aqui: `LACUNA-OFF-001`, `LACUNA-OFF-010` e, em 2026-09-23,
`LACUNA-OFF-004` e `LACUNA-OFF-006`; `LACUNA-OFF-007` fechou na parte de número.
...
- **`LACUNA-OFF-002`** — é **admissível** pré-alocar faixa de numeração fiscal por terminal? A numeração é sequencial por estabelecimento e por série, subsérie é vedada e a série separa processo de emissão (`RN-EMI-022`, `RN-EMI-023`); se a faixa por terminal exigir **série por ponto de emissão**, muda habilitação, custo e o desfecho do não usado. **Dono:** humano (com o contador). Bloqueia o mecanismo, não o requisito.
  ...
  > estabelecimento e série, sem presumir série por terminal (`RN-NUC-065`, item 3). Desde: 2026-09-23.
- **`LACUNA-OFF-003`** — mecanismo de alocação de faixa e **formato** da identidade cunhada no terminal (`RN-OFF-006`, `RN-OFF-013`). **Corrigido em 2026-09-23:** dizia que `D-04` estava ABERTA; ela fechou em 2026-09-11 ([[decision-d-04-chave-timestamps-exclusao]]) e a identidade é `uuid` ordenado no tempo. O que resta é o mecanismo de faixa. **Dono:** `arquiteto-dados`, Fase 1.
- **`LACUNA-OFF-004` — FECHADA em 2026-09-23** → `RN-OFF-037` ([[decision-valores-de-partida-da-continuidade-offline]]). Perguntava o **número** do teto de operação offline, em **vendas concluídas pendentes** (`RN-OFF-014`), e o aviso de aproximação.
...
- **`LACUNA-OFF-006` — FECHADA em 2026-09-23** → `RN-NUC-057` ([[decision-fuso-e-moeda-sao-do-estabelecimento]]). Era o **conflito de fuso**: `RN-FIS-008` resolvia vigência no fuso do **estabelecimento**, e `glossario.md` §1.1 e `.claude/rules/dados.md` §3 tratavam o fuso como dado do **cliente (tenant)**; de qual hora local o terminal offline atribui versão de regra? Do estabelecimento, publicado com vigência. `RN-OFF-019` e a linha de `FIS` da §4 foram corrigidas na mesma data.
- **`LACUNA-OFF-007`** — **qual papel** é dono da fila de pendências, e qual a escalada quando ninguém olha (`RN-OFF-011`; `RN-EMI-028` já pedia e ninguém atribuiu). **Dono:** humano; entrada direta de **D-03**. Respondida na parte de papel (`queue_owner`, `RN-OFF-011`) e, em 2026-09-23, na de número (`LACUNA-NUC-014` → `RN-OFF-040`).
...
**Perguntas para o humano:** as quatro que estavam aqui foram respondidas, e ficam para o histórico: (1) teto e exceção → `RN-OFF-037`; (2) dono da fila → `queue_owner` (`RN-OFF-011`), e a escalada → `RN-OFF-040`; (3) fuso → estabelecimento (`RN-NUC-057`); (4) validade do papel retido → `RN-OFF-034`. As de número foram por decisão delegada em 2026-09-23 (`T-0014`). Seguem abertas `LACUNA-OFF-002` e `LACUNA-OFF-009`, as duas com o contador.
